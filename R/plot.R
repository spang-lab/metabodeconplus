...
#'
#' @seealso
#' [metabodeconplus::plot_spectrum()] for a much more sophisticated plotting routine
...
plot_spectra <- function(
    ...
    # supal is cleared by snap_to_ref (post-snap), so fall back to sup/si.
...
#' [metabodeconplus::si_mat()] and pass the result.
#'
#' @param foc_rgn
#' Numeric vector of length 2 specifying the focus region in ppm
#' (e.g. `c(3.55, 3.35)`). If NULL (default), the full spectrum is shown.
...
#' a sparse feature matrix from [metabodeconplus::si_mat()].
#'
#' @param true_col
...
#' details see [metabodeconplus::metabodeconplus-classes].
#'
#' @param ...
#' Additional arguments passed to [metabodeconplus::draw_spectrum()] for **every** sub figure.
#' See 'Details'.
#'
#' @param obj
...
#' List of arguments passed to [metabodeconplus::draw_spectrum()] when drawing sub figure
#' 1-3. See 'Details'.
#'
#' @param mar
...
#' [metabodeconplus::draw_spectrum()] multiple times to draw the following sub figures  onto  the
#' plotting canvas:
#'
...
#'
#' The  argument  lists  for  the  individual  calls  to  [metabodeconplus::draw_spectrum()]  are
#' determined at runtime and depend on the arguments passed to [metabodeconplus::plot_spectrum()]
...
#' [metabodeconplus::draw_spectrum()] by providing  a  corresponding  named  element  in  `sub1`,
#' `sub2` or `sub3`.
#'
...
#' @title Draw Spectrum
#' @description
#' Draws a single spectrum.  Internally  used  by  [metabodeconplus::plot_spectrum()],  which  is
#' usually  the  recommended  way  to  plot  spectra.  For  usage  examples  see
#' [test/testthat/test-draw_spectrum.R](
#' https://github.com/spang-lab/metabodeconplus/blob/main/tests/testthat/test-draw_spectrum.R).
#'
#' `r lifecycle::badge("experimental")`
#'
#' @param obj
#' An object of type `spectrum` or `decon2`. For details see [metabodeconplus::metabodeconplus-classes].
#'
#' @param add
...
test_draw_spectrum <- function(figs = 1:8,
                               ...
                               oma = c(2, 2, 2, 2) + 0.1,
                               store = FALSE) {
    spectra <- metabodeconplus::sim[1:4] # Lazydata is not available in the package namespace
