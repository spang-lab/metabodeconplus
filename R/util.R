...
#' @keywords internal
#' @useDynLib metabodeconplus, .registration=TRUE
"_PACKAGE"
...
aaa_Get_Started <- function(open_browser = interactive()) {
    url <- "https://spang-lab.github.io/metabodeconplus/articles/Get_Started.html"
...
#' pkg_file()
pkg_file <- function(...) {
    system.file(..., package = "metabodeconplus")
}
...
#'
#' @param x
#' A `spectrum` object as described in [metabodeconplus::metabodeconplus-classes].
#'
#' @return
...
get_worker_pool <- function(nw, loadpkg = TRUE, log = TRUE) {
    ...
    cl <- makeCluster(nw)
    if (loadpkg) {
        expr <- parse(text = 'attach(asNamespace("metabodeconplus"))')
...
loaded_via_devtools <- function() {
    pkg_dir <- dirname(system.file("DESCRIPTION", package = "metabodeconplus"))
