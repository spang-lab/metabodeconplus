...
* Skipped the `glmnet`/lasso `fit_mdm()` and `benchmark()` smoke tests on
  Windows to fix a flaky `R CMD check` failure. `glmnet`'s compiled code
  ...
  unaffected, and its tests continue to run on all platforms (including
  Windows). The lasso tests still run on Linux and macOS.
