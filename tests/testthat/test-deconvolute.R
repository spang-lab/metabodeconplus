...
rust_backend <- test_that("deconvolute works with rust backend", {
    ...
    expect_sap1_deconvolution_failed(rt_mm, "Using the Rust backend.*requires the optional 'mdrb'")
    expect_match(attributes(rt_mm)$condition$message, 'install.packages("mdrb"', fixed=TRUE)
